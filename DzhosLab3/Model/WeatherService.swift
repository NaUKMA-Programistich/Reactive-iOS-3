...
class WeatherService {
    ...
    private func getForecast(lat: String, lon: String) -> Single<ForecastDataDto> {
    ...
    private func getCoordinate(query: String) -> Single<Location> {
    ...
    private func getImage(icon: String) -> Single<(String, Data)> {
        return Single<(String, Data)>.create { single in
            ...
            let task = URLSession.shared.dataTask(with: URL(string: url)!) { data, _, error in
                ...
                // self.logger.info("#getImage we have data \(data)")
